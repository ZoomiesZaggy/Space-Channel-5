...
class PlayerConfigTests(unittest.TestCase):
    def test_roundtrip_path_with_spaces_and_equals(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            values = dict(DEFAULTS, gdi='D:/My Games/Disc = USA.gdi', native_mod='C:/My Mods/test = 1.dll', volume='0', key_a='Q')
            save(path, values)
            self.assertEqual(load(path), values)
            self.assertFalse(path.with_suffix('.tmp').exists())

    def test_rejects_injection_and_ambiguous_bindings(self):
        for change in ({'gdi': 'x\nvolume=0'}, {'key_a': 'X'}, {'pad_a': 'b'}, {'volume': '-1'}, {'volume': '101'}, {'fullscreen': '2'}, {'key_a': 'FakeKey'}, {'unknown': '1'}, {'native_mod': 'relative.dll'}, {'native_mod': 'C:relative.dll'}):
