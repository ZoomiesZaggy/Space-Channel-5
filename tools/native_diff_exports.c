...
#undef main
__declspec(dllexport) void sc5_reset(const unsigned char *image){
...
__declspec(dllexport) void sc5_run(u32 budget){run(budget);}
/* Relative chunks avoid the diagnostic's 32-bit absolute instruction limit. */
__declspec(dllexport) u32 sc5_run_chunk(u32 budget){steps=0;run(budget);return steps;}
__declspec(dllexport) u32 sc5_state(u32 index){
...
__declspec(dllexport) const unsigned char *sc5_ram(void){return ram;}
__declspec(dllexport) State *sc5_context(void){return &s;}
__declspec(dllexport) void sc5_set_retire(Retire callback){external_retire=callback;}
__declspec(dllexport) void sc5_set_fast_clock(NativeFastClock *clock){
...
__declspec(dllexport) void sc5_set_service(Service callback){external_service=callback;}
__declspec(dllexport) void sc5_set_queue_write(QueueWrite callback){external_queue_write=callback;}
__declspec(dllexport) void sc5_div1(u32 n,u32 m){divide_step(n,m);}
__declspec(dllexport) void sc5_float(u32 n,u32 m,u32 kind){float_arithmetic(n,m,kind);}
__declspec(dllexport) void sc5_float_extended(u32 n,u32 m,u32 kind){float_extended(n,m,kind);}
__declspec(dllexport) void sc5_fmov(u32 n,u32 m,u32 kind){fmov_single(n,m,kind);}
__declspec(dllexport) void sc5_transform(u32 n){float_transform(n);}
__declspec(dllexport) void sc5_sincos(u32 n){float_sincos(n);}
__declspec(dllexport) void sc5_inner(u32 n,u32 m){float_inner(n,m);}
__declspec(dllexport) void sc5_precision(u32 n,u32 direction){float_precision(n,direction);}
__declspec(dllexport) void sc5_convert(u32 n,u32 kind){float_convert(n,kind);}
__declspec(dllexport) void sc5_use_bus(unsigned char *shared_ram,BusRead read,BusWrite write){
...
__declspec(dllexport) void sc5_attach_bus(unsigned char *shared_ram,BusRead read,BusWrite write){
